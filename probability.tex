...
The so-called \textit{one-sample t-test} is used to determine whether the mean of a single sample differs significantly from a known or hypothesized population mean. It assumes that the sample data are drawn from a normally distributed population. The test statistic is given by:
\[
...
\[
    f(x; d_{1}, d_{2}) = \frac{s_1^{2}}{s_2^{2}},
\]
...
The so-called Analysis of Variance, or just \textit{ANOVAt}, is used to determine whether the variation of a dataset comes primary from variation within the samples themselves, or from variation between the groups. It is an special case of the Fisher test, where the F statistic is computed as:
\[
    f(x; d_{1}, d_{2}) = \frac{s_between^{2}}{s_within^{2}},
\]
where $s_1^{2}$ and $s_2^{2}$ are the sample variances, and the degrees of freedom are $d_1 = n_1 - 1$ and $d_2 = n_2 - 1$.\\
