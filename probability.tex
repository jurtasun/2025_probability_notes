...
\epigraph{\textit{Statistics is the grammar of science.}}{— Karl Pearson}
 ...
 A large part of history of science could be summarized as a record of the continuous human striving for a formulation of concepts which permit description of the real world in mathematical terms. To define mathematically some part of the natural world and phenomena we see, it is necessary to develop an appropriate mathematical model relating the one or more relevant quantities - sometimes called \textit{variables}, cause they relate and change depending on one another. The purpose of modelling might be, for instance, to determine the distance of the earth to the sun, to estimate the number of stars [...], or - deeply related to statistical sciences, relate the boiling point of water to the external pressure. Or relate number of cancer patients to smoking areas, or pollution levels [...].\\
 
 Colombian mathematician Lucho Recalde explained that mathematics can be reduced to count, measure, and sort. When it comes to the description of chance, variation and uncertainty, the fields of statistics and probability add the idea of randomness, relation and correlation [...], confidence and reproducibility.
...
The \textit{sample} on the other hand, is the finite subset actually collected. For a series of $N$ observations $x_1$, $x_2$, ..., $x_N$, a sample of just $n$ elements - less than the total, denoted by the upper case $N$ - is defined as
\begin{equation}
	\mathcal{S} = \{x_{i_1}, x_{i_2}, \dots, x_{i_n}\}, \quad n \ll N \; ,
\end{equation}

where the $i$-subscripts remind us that the sample consists of selected observations from the population, not necessarily consecutive or all of them. The population represents the ideal object of inference, while the sample is the concrete, finite evidence available to us. This distinction is far from trivial; a poorly chosen sample misrepresents the population and may induce bias, whereas a carefully constructed one mirrors its essential features, and can be used to describe the underlying nature.

\medskip

Equally important is the recognition that not all data are of the same kind. \textit{Categorical}, or \textit{qualitative} data describes qualities or labels such as the eye colour of students in a classroom (blue, brown, green), the brand of a purchased smartphone, etc. Some times they are further divided into \textit{nominal} categories, with no natural order, the eye colour or the smartphone brand, and \textbf{ordinal} categories with a meaningful order. Examples of these would be the finishing places in a race (first, second, third), survey responses ranging from “strongly disagree” to “strongly agree”, etc.\\
\indent The other big group is normally referred to as \textit{numerical}, or \textit{quantitative} data. These measure quantities and can also be subdivided into \textbf{discrete}, countable numbers, e.g., the number of books on a shelf (4, 5, 6) or the number of goals scored in a match, and \textbf{continuous} values that can take any number within a range, e.g., the time a sprinter takes to run 100 meters, or the height of a person measured with arbitrary precision.

\medskip

Distinguishing between these types is no mere pedantry; it shapes the tools one may legitimately apply. It would not make sense to compute a mean of smartphone brands, but to compute the mean of their prices is informative. Similarly, the distribution of finishing places after a race might be summarized by a median position, whereas heights of athletes could be studied with averages and measures of spread. A correct classification of data is thus a safeguard against misuse and a guide toward insight.

\medskip

Sampling and proper description of data establish the ground upon which of statistics is built. Before calculating, summarizing, or diving into inference, one must ensure that the information collected is both representative and properly understood. Without these foundations, descriptive measures risk floating unmoored, detached from the reality they claim to represent. Accurate sampling and rigorous description of the quantities under study will lead to a faithful representation of the phenomena under study and their relations, detecting anomalies, and even building accurate predictions.
...
While numerical summaries useful, the human mind often understands patterns much faster through vision than calculation. Data visualization transforms sequences of numbers into shapes, colours and structures that can be grasped at a glance. It turns abstraction into perception and often reveals regularities invisible to formulas alone. Nowadays, the sole filed of \textit{data visualization} has become one of the pillars of any scientific or data related topic.

\medskip

Among the oldest and most fundamental visualization tools is the \textit{histogram}. The concept of dividing data into intervals to visualize frequency dates back to Karl Pearson in the late 19th century (1891) \cite{pearson1892}, who formalized it as a graphical representation of probability distributions. A histogram divides the range of a dataset into consecutive intervals, or \textit{bins}, and represents the frequency of observations falling within each bin as the height of a bar. This simple yet powerful plot provides an immediate visual impression of the dataset’s distribution, allowing one to identify symmetry, skewness, concentration of values, and potential gaps. For example, a symmetric histogram, like the one in figure [...] suggests a roughly balanced distribution around the mean, while a right-skewed histogram, like the one displayed in [...], indicates that higher values are less frequent but can influence measures like the mean. This is the state of the art in physical sciences and whenever data is supposed to fit a mathematical prediction. For instance, in figure [...] we can see a histogram which is very well described by a linear fit, with a small deviation in [...]. Figure [...] shows.

\medskip

The box plot, also known as the box-and-whisker plot, was introduced by John Tukey in 1970 as part of his work on exploratory data analysis \cite{tukey1977}. The box plot offers a compact summary of a dataset’s central tendency, spread, and potential outliers. Constructed from five key statistics - the minimum, first quartile ($Q_1$), median ($Q_2$), third quartile ($Q_3$), and maximum - it clearly shows the interquartile range (IQR = $Q_3 - Q_1$) and highlights points that fall outside 1.5 times the IQR as outliers. This representation allows for quick comparisons across multiple groups, and it is particularly useful for detecting asymmetry and variability without being overly influenced by extreme values. It is widely used in biological and clinical sciences where an experiment can be repeated many times with relatively small sizes.

\medskip

The violin plot is a more recent innovation, combining the box plot with a kernel density estimate to visualize the shape of the distribution. While the precise origin is less formally documented, it gained prominence in the late 20th century in statistical software environments, such as R, during the 1990s. \cite{hintze1997} The violin plot extends the concept of the box plot by combining it with a kernel density estimate of the data. This plot not only displays the median and quartiles but also provides a smooth depiction of the distribution’s shape, revealing features such as multimodality or skewness that might be obscured in a simple box plot. By showing both summary statistics and the underlying density, the violin plot gives a richer, more nuanced view of the dataset, particularly when comparing several groups side by side.

\medskip

Underlying these graphics is the concept of \textit{quantiles}. The median is the 50th percentile, quartiles mark the 25th and 75th percentiles, and finer partitions yield deciles or percentiles. Mathematically, the $p$-quantile of a dataset is the value $q$ p such that at least a proportion $p$ of the data lies below it. Quantiles give a language for describing position within a distribution.

\medskip

Graphs and diagrams are not mere decoration but instruments of analysis. They allow patterns to leap from obscurity, invite hypotheses, and sometimes contradict our assumptions. In practice, visualization is both a beginning and a test: a first impression of data, and a final check on the reasonableness of results derived through calculation.
...
Data seldom lives in isolation. More often, one variable depends upon another: rainfall influences crop yields, study hours affect exam results, and economic indicators move in concert. Recognizing and describing such dependencies lies at the heart of descriptive statistics and prepares the way for predictive models.

\medskip

The simplest and most widely studied form of dependency is linearity. When one variable tends to increase in proportion to another, the relation can be sketched as a straight line. In regression, the best-fitting line is expressed as
\begin{equation}
	y = a + b \: x
	\label{eq:linear}
\end{equation}

where $a$ is the intercept and $b$ the slope, chosen to minimize the sum of squared deviations between observed and predicted values.

\medskip

Correlation quantifies the strength and direction of this relationship. The \textit{Pearson correlation coefficient} $\rho_{x, y}$, sometimes called $r$
\begin{equation}
	r = \rho_{x, y} = \frac{\text{cov}(x, y)}{\sigma_x \: \sigma_y} \; ,
	\label{eq:correlation}
\end{equation}

where $\textit{cov}$ is the covariance, defined in terms of the expectation value
\begin{equation}
	cov(x, y) = \mathbb{E}[(x - \mu_x) \; (y - \mu_y)]
\end{equation}

ranges between -1 and +1, measuring how tightly the points cluster around a line. A value near +1 suggests strong positive association, near -1 strong negative association, and near 0 little linear relationship.
...
(Contains the introduction of the term and concept of histogram).

\bibitem{tukey1977}
Tukey, J.W. (1977). 
\"Exploratory Data Analysis.\" 
Addison-Wesley. 
(First book introducing the box plot as a graphical method).

\bibitem{hintze1997}
Hintze, J.L., \& Nelson, R.D. (1997).
\"Violin Plots: A Box Plot-Density Trace Synergism.\" 
The American Statistician, 52(2), 181–184. 
doi:10.1080/00031305.1998.10480559
(Introduced the violin plot in statistics).
