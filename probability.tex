...
Correlation is a fundamental measure of association between two random variables, quantifying how strongly they vary together. The most widely used form is the \textit{Pearson correlation coefficient}, introduced by Karl Pearson in the 1890s, which is built upon the idea of covariance normalized by variability. For two random variables $X$ and $Y$, the population correlation $\rho$ is defined as
\begin{equation}
	\rho = \frac{\mathrm{Cov}(X, Y)}{\sigma_X \, \sigma_Y}
\end{equation}
where the covariance is [...], hence the correlation remains
\begin{equation}
	\rho = \frac{\mathbb{E}\big[(X - \mu_X)(Y - \mu_Y)\big]}{\sqrt{\mathbb{E}\big[(X - \mu_X)^2\big]}\sqrt{\mathbb{E}\big[(Y - \mu_Y)^2\big]}} ,
\end{equation}

where $\mu_X, \mu_Y$ are the expected values and $\sigma_X, \sigma_Y$ the standard deviations of $X$ and $Y$. By construction, $\rho$ takes values in $[-1,1]$, with $+1$ indicating perfect positive linear association, $-1$ perfect negative association, and $0$ the absence of linear correlation.

In practice, population parameters are unknown, and we compute a sample correlation $r$ from observed data. For a dataset of $n$ paired observations $\{(x_i, y_i)\}_{i=1}^n$, the formula becomes
\begin{equation}
	r = \frac{\sum_{i=1}^n (x_i - \overline{x})(y_i - \overline{y})}{\sqrt{\sum_{i=1}^n (x_i - \overline{x})^2} \, \sqrt{\sum_{i=1}^n (y_i - \overline{y})^2}} ,
\end{equation}
where $\overline{x}$ and $\overline{y}$ are the sample means. This expression reveals $r$ as the normalized sample covariance, measuring co-movement relative to the independent variation of each variable.

\medskip

To illustrate, consider a small dataset of study hours $X$ and exam scores $Y$ for four students:
\begin{equation}
	(2, \, 65), \quad (4, \, 70), \quad (6, \, 75), \quad (8, \, 85).
\end{equation}
The sample means are $\overline{x} = (2+4+6+8)/4 = 5$ and $\overline{y} = (65+70+75+85)/4 = 73.75$. Computing the numerator,
\begin{equation}
	\sum_{i=1}^4 (x_i - \overline{x})(y_i - \overline{y}) = (2-5)(65-73.75) + (4-5)(70-73.75) + (6-5)(75-73.75) + (8-5)(85-73.75) = 65.
\end{equation}
The denominators are
\[
\sqrt{\sum_{i=1}^4 (x_i - \overline{x})^2} = \sqrt{9+1+1+9} = \sqrt{20}, \quad 
\sqrt{\sum_{i=1}^4 (y_i - \overline{y})^2} = \sqrt{76.56+14.06+1.56+126.56} \approx \sqrt{218.75}.
\]
Hence
\begin{equation}
	r = \frac{65}{\sqrt{20} \cdot \sqrt{218.75}} \approx 0.98.
\end{equation}

\medskip

This high positive correlation ($r \approx 0.98$) reflects the intuitive fact that, in this dataset, students who study more hours tend to achieve higher exam scores. Such a simple example shows how Pearson's $r$ provides a precise, normalized measure of linear association, bridging the abstract definition in terms of expectations with concrete calculations on observed data.
